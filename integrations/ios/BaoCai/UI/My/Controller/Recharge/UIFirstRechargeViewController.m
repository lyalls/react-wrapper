    ...
    [MyRequest checkRechargeBankCardNumWithCardNum:cardNumStr success:^(NSDictionary *dic, BCError *error) {
        if (error.code == 0) {
            if ([dic boolForKey:@"type"]) {
                NSString *bankCode = [[dic objectForKey:@"list"] objectForKey:@"bankCode"];
                NSArray *array = [UserDefaultsHelper sharedManager].bankSupportList;
                BOOL isHasBankCode = NO;
                for (NSDictionary *dic in array) {
                    if ([[dic objectForKey:@"llcode"] isEqualToString:bankCode]) {
                        isHasBankCode = YES;
                    }
                }
                if (isHasBankCode) {
                    [MyRequest rechargeWithAmount:rechargeStr withCardNum:cardNumStr withNoAgree:@"" success:^(NSDictionary *dic, BCError *error) {
                        HIDDENPROGRESSHUD;
                        if (error.code == 0) {
                            UIRechargeResultViewController *viewController = [self getControllerByStoryBoardType:StoryBoardTypeRecharge identifier:@"UIRechargeResultViewController"];
