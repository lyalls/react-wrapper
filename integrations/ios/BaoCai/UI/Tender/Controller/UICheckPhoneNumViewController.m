...
- (void)viewDidLoad {
    ...
    [self.getVerfiyBtn setBorder:OrangeColor width:0.5];
...
- (IBAction)getVerfiyBtnClick:(id)sender {
    ...
    [UserRequest retrieveTraderPasswordOneWithPhoneNum:[UserInfoModel sharedModel].phone success:^(NSDictionary *dic, BCError *error) {
        if (error.code == 0) {
            ...
            sign = [[NSString stringWithFormat:@"%@%@", sign, [UserInfoModel sharedModel].phone] sha1Encrypt];
...
- (IBAction)nextBtnClick:(id)sender {
    ...
    [UserRequest retrieveTraderPasswordCheckVerfiyCodeWithPhoneNum:[UserInfoModel sharedModel].phone codeStr:verfiyStr success:^(NSDictionary *dic, BCError *error) {
        HIDDENPROGRESSHUD;
        if (error.code == 0) {
            UIResetTraderPasswordViewController *view = [self getControllerByStoryBoardType:StoryBoardTypeTender identifier:@"UIResetTraderPasswordViewController"];
