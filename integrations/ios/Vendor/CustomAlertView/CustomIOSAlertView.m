...
- (void)show:(NSString*)title message:(NSString*)message buttonTitles:(NSArray*)titles
{
    //计算高度
    UILabel* lbmsg = [[UILabel alloc] initWithFrame:CGRectMake(0, 35, 300, 80)];
    lbmsg.text = message;
    lbmsg.font = [UIFont systemFontOfSize:14];
    lbmsg.textColor = [UIColor blackColor];
    lbmsg.numberOfLines = 0;
    lbmsg.textAlignment = NSTextAlignmentCenter;
    NSDictionary *attribute = @{NSFontAttributeName: lbmsg.font};
    CGSize retSize = [message boundingRectWithSize:CGSizeMake(300, 100)
                                             options:
    ...
    {
        ht = 90 + retSize.height;
    }
    
    UIView* view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 300, ht)];
    UILabel* lbtitle = [[UILabel alloc] initWithFrame:CGRectMake(0,20, 300, 40)];
...
- (void)show:(NSString*)title message:(NSString*)message
{
    ...
    UILabel* lbtitle = [[UILabel alloc] initWithFrame:CGRectMake(0,10, 280, 40)];
    lbtitle.font = [UIFont boldSystemFontOfSize:17];
    lbtitle.textColor = [UIColor blackColor];
    lbtitle.text = title;
    lbtitle.textAlignment = NSTextAlignmentCenter;
    ...
    UILabel* lbmsg = [[UILabel alloc] initWithFrame:CGRectMake((kScreen_Width-210)/2, 50, 200, 80)];
    ...
    [msg addAttribute:NSParagraphStyleAttributeName value:paragraphStyle1 range:NSMakeRange(0, [message length])];
    [msg addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:13] range:NSMakeRange(0, [message length])];
    [view addSubview:lbmsg];
    [lbmsg setAttributedText:msg];
    CGSize size = [msg boundingRectWithSize:CGSizeMake(200, 300) options:NSStringDrawingTruncatesLastVisibleLine |
    NSStringDrawingUsesLineFragmentOrigin |
    NSStringDrawingUsesFontLeading context:nil].size;
    
    CGFloat hh = size.height;
    
    view.frame = CGRectMake(0, 0, 280, hh+75);
    lbmsg.frame = CGRectMake((280 - size.width)/2 -10, 50, size.width + 30,hh+10);
    self.containerView = view;
    self.buttonTitles = @[@"确  定"];
    [self setUseMotionEffects:true];
    [self show];
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        if(i>0)
        {
            UIView* sp = [[UIView alloc] initWithFrame:CGRectMake(i * buttonWidth, container.bounds.size.height - buttonHeight, 1, buttonHeight)];
            sp.backgroundColor = [UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0f];
